...
void Bot::recordMove(Move* move, PlaySide sideToMove) {
    ...
    if (move->isDropIn()) {
      if (sideToMove == WHITE) {
        if (move->replacement.value() == 4) {
          board[iDest][jDest] = 'Q';
        } else if (move->replacement.value() == 1) {
          board[iDest][jDest] = 'R';
        } else if (move->replacement.value() == 2) {
          board[iDest][jDest] = 'B';
        } else if (move->replacement.value() == 3) {
          board[iDest][jDest] = 'N';
        }
    ...
    if (move->source.value() == getPos(0, 4) && move->destination.value() == getPos(0, 6)) {
      board[0][5] = 'r';
      board[0][7] = EMPTY;
    } else if (move->source.value() == getPos(0, 4) && move->destination.value() == getPos(0, 2)) {
      board[0][3] = 'r';
      board[0][0] = EMPTY;
    } else if (move->source.value() == getPos(7, 4) && move->destination.value() == getPos(7, 6)) {
      board[7][5] = 'R';
      board[7][7] = EMPTY;
    } else if (move->source.value() == getPos(7, 4) && move->destination.value() == getPos(7, 2)) {
      board[7][3] = 'R';
      board[7][0] = EMPTY;
    }
...
vector<Move*> Bot::getDropIn(PlaySide sideToMove) {
  vector<Move*> moves;
  int i, j;
  cout << "aloo" << endl;
  if (sideToMove == WHITE) {
    ...
    char c = pwhite.front();
    pwhite.erase(pwhite.begin());
    //move throw the board and check for empty space
    //!check later
    for (i = 1; i < 7; i++) {
      for (j = 0; j < 8; j++) {
        if (board[i][j] == EMPTY) {
          if (c == 'q') {
            moves.push_back(Move::dropIn(getPos(i, j), (Piece)4));
            return moves;
          }
          else if (c == 'r') {
            moves.push_back(Move::dropIn(getPos(i, j), (Piece)1));
            return moves;
          }
          else if (c == 'b') {
            moves.push_back(Move::dropIn(getPos(i, j), (Piece)2));
            return moves;
          }
          else if (c == 'n') {
            moves.push_back(Move::dropIn(getPos(i, j), (Piece)3));
            return moves;
          }
          else if (c == 'p') {
            moves.push_back(Move::dropIn(getPos(i, j), (Piece)0));
            return moves;
          }
        }
...
Move* Bot::calculateNextMove() {
  ...
  if (moves.size() == 0) {
   ...
   Move* move;
    recordMove(moves[random], playSide);
  if (moves[random]->isPromotion()) {
    move = Move::promote(moves[random]->source.value(), moves[random]->destination.value(), moves[random]->replacement.value());
  } else if (moves[random]->isDropIn()) {
    move = Move::dropIn(moves[random]->destination.value(), moves[random]->replacement.value());
  } else {
